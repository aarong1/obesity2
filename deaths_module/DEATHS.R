...
past_populations <- past_populations[intervention == 'non-intervention',]

range(past_populations$year)
past_populations=past_populations[year<=(min(past_populations$year)+1),]
range(past_populations$year)
cut_yr <- min(past_populations$year)+1

past_populations %>% 
  # filter(year == 2025) %>%
  count(year, asthma,run) %>% 
  mutate(csum = cumsum(n) - first(n))

past_populations = past_populations[year==min(year),]
...
#prevalence
#STROKE
# setDT(population_w_established_prevalence)

stroke <- past_populations[year <= cut_yr,
               .N,
               by = .(run,year,stroke)
               ][order(run)
][,  .(cases=mean(N)*model_specification$population$scale_down_factor), by = .(year,stroke)
  ][, csum := cumsum(cases) - first(cases), by=year]

stroke[stroke==min(year),cases]
stroke[stroke==min(year),csum]

d[d$std=='stroke','deaths']/2 / stroke[stroke==(min(year)),cases]
d[d$std=='stroke','deaths']/2 / stroke[stroke==min(year),csum]

#CHD

chd <- past_populations[year <= cut_yr,
                           .N,                    
                           by = .( year,run,chd)
][order(run)
][,  .(cases=mean(N)*model_specification$population$scale_down_factor), by = .(year,chd)][, csum := cumsum(cases) - first(cases), by=year]

chd[chd==min(year),cases]
chd[chd==min(year),csum]

d[d$std=='CHD','deaths']/2 / chd[chd==(min(year)),cases]
d[d$std=='CHD','deaths']/2 / chd[chd==min(year),csum]

#DIABETES

# diabetes[diabetes !=0,sum(N)]
diabetes <- past_populations[year <= cut_yr,
                       .N,                    
                       by = .(run,diabetes)
][order(run)        
...
d[d$std=='diabetes','deaths']/ diabetes[diabetes==max(diabetes),csum]
#0.004650208

# asthma
# asthma[asthma !=0,sum(N)]
asthma <- past_populations[year <= cut_yr,
                             .N,                    
                             by = .(run,asthma)
][order(run,asthma)        
...
d[d$std=='asthma','deaths']/ asthma[asthma==max(asthma),csum]

# cancer
# cancer[cancer !=0,sum(N)]
cancer <- past_populations[year <= cut_yr,
                           .N,                    
                           by = .(run,cancer)
][order(run,cancer)        
][,  .(cases=mean(N)*model_specification$population$scale_down_factor), by = .(cancer)][, csum := cumsum(cases) - first(cases)
]

cancer[max(cancer,na.rm = T)==cancer,csum]

d[d$std=='cancer','deaths']/ cancer[cancer==max(cancer),csum]

# copd
# copd[copd !=0,sum(N)]
copd <- past_populations[year <= cut_yr,
                           .N,                    
                           by = .(run,copd)
][order(run,copd)        
][,  .(cases=mean(N)*model_specification$population$scale_down_factor), by = .(copd)][, csum := cumsum(cases) - first(cases)]

prevalence_hsct %>% filter(Year==max(Year)) %>%
  count(Disease,wt = Count)

copd[max(copd)==copd,csum]

d[d$std=='COPD','deaths']/ copd[copd==max(copd),csum]

# dementia
# dementia[dementia !=0,sum(N)]
dementia <- past_populations[year <= cut_yr,
                         .N,                    
                         by = .(run,dementia)
][order(run,dementia)        
...
d[d$std=='dementia','deaths']/ dementia[dementia==max(dementia),csum]

# heart_failure
# heart_failure[heart_failure !=0,sum(N)]
heart_failure <- past_populations[year <= cut_yr,
                             .N,                    
...
d[d$std=='heart_failure','deaths']/ heart_failure[heart_failure==max(heart_failure),csum]

# chronic_kidney_disease
# chronic_kidney_disease[chronic_kidney_disease !=0,sum(N)]
chronic_kidney_disease <- past_populations[year <= cut_yr,
                             .N,
...
chronic_kidney_disease[chronic_kidney_disease==max(chronic_kidney_disease),csum]

d[d$std=='chronic_kidney_disease','deaths']/ chronic_kidney_disease[chronic_kidney_disease==max(chronic_kidney_disease),csum]
...
case_fatality_rate_df <- tribble(
  ...
  'cancer',
  as.numeric(d[d$std=='cancer','deaths'] / cancer[cancer!=0, sum(cases)]),
  as.numeric(d[d$std=='cancer','deaths']),
  cancer[cancer!=0, sum(cases)],
  'Malignant neoplasms - deaths attributed to prevalence'#,
  # 'cancer', lung_case_fatality,NA, NA, NA,
...
cancer_rate <- case_fatality_rate_df %>% 
  filter(morbidity=='cancer') %>% 
  pull(case_fatality_rate)


case_fatality_rate_df
cancer_rate
site_cancer_case_fatality_rates <- tribble(
~morbidity, ~case_fatality_rate, ~deaths,  ~cases, ~notes,     
"lung_cancer" , cancer_rate , NA, NA ,NA,
"prostate_cancer", cancer_rate , NA, NA ,NA,
"colorectal_cancer", cancer_rate , NA, NA ,NA,
"oral_cancer", cancer_rate , NA, NA ,NA,
"female_breast_cancer", cancer_rate , NA, NA ,NA,
"pancreatic_cancer", cancer_rate , NA, NA ,NA,
"uterine_cancer", cancer_rate , NA, NA ,NA,
"blood_cancer", cancer_rate , NA, NA ,NA,
"ovarian_cancer", cancer_rate , NA, NA ,NA,
"osteogastric_cancer", cancer_rate , NA, NA ,NA,
"renal_cancer", cancer_rate , NA, NA ,NA
)


case_fatality_rate_df <- rbind(
case_fatality_rate_df,
site_cancer_case_fatality_rates
)
...
# Example usage with the updated function
past_populations %>% filter(year == min(year)) %>% setDT() %>% 
  apply_case_death(morbidity = 'diabetes') %>% 
