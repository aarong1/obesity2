# =============================================================================
# Populate All Cancers Prevalence
# =============================================================================
# Function to assign All Cancers prevalence to a synthetic population
...
#' Populate All Cancers prevalence in synthetic population
#' 
...
#' @return input_population with added column: all_cancers (year of diagnosis or 0)
#' @export
#' 
...
populate_all_cancers_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for All Cancers")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), cancer)
  
  # Report summary
  n_cases <- sum(result$cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "All Cancers", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with All Cancers (10-year prevalence)
  pop_with_cancer <- populate_all_cancers_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$all_cancers > 0)
