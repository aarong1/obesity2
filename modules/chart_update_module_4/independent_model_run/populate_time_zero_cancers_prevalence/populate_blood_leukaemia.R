# =============================================================================
# Populate Blood Leukaemia Prevalence
# =============================================================================
# Function to assign Blood Leukaemia prevalence to a synthetic population
...
#' Populate Blood Leukaemia prevalence in synthetic population
#' 
...
#' @return input_population with added column: blood_leukaemia (year of diagnosis or 0)
#' @export
populate_blood_leukaemia_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Blood Leukaemia")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), blood_leukaemia)
  
  # Report summary
  n_cases <- sum(result$blood_leukaemia > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Blood Leukaemia", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Blood Leukaemia (10-year prevalence)
  pop_with_cancer <- populate_blood_leukaemia_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$blood_leukaemia > 0)
