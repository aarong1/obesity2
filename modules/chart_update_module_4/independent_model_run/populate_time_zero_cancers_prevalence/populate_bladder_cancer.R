# =============================================================================
# Populate Bladder Cancer Prevalence
# =============================================================================
# Function to assign Bladder Cancer prevalence to a synthetic population
...
#' Populate Bladder Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: bladder_cancer (year of diagnosis or 0)
#' @export
populate_bladder_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Bladder Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), bladder_cancer)
  
  # Report summary
  n_cases <- sum(result$bladder_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Bladder Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Bladder Cancer (10-year prevalence)
  pop_with_cancer <- populate_bladder_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$bladder_cancer > 0)
