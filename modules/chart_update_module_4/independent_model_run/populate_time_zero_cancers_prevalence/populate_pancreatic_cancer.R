# =============================================================================
# Populate Pancreatic Cancer Prevalence
# =============================================================================
# Function to assign Pancreatic Cancer prevalence to a synthetic population
...
#' Populate Pancreatic Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: pancreatic_cancer (year of diagnosis or 0)
#' @export
populate_pancreatic_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Pancreatic Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), pancreatic_cancer)
  
  # Report summary
  n_cases <- sum(result$pancreatic_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Pancreatic Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Pancreatic Cancer (10-year prevalence)
  pop_with_cancer <- populate_pancreatic_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$pancreatic_cancer > 0)
