# =============================================================================
# Populate Oral Cancer Prevalence
# =============================================================================
# Function to assign Oral Cancer prevalence to a synthetic population
...
#' Populate Oral Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: oral_cancer (year of diagnosis or 0)
#' @export
populate_oral_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Oral Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), oral_cancer)
  
  # Report summary
  n_cases <- sum(result$oral_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Oral Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Oral Cancer (10-year prevalence)
  pop_with_cancer <- populate_oral_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$oral_cancer > 0)
