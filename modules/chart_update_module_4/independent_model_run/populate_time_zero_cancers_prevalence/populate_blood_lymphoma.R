# =============================================================================
# Populate Blood Lymphoma Prevalence
# =============================================================================
# Function to assign Blood Lymphoma prevalence to a synthetic population
...
#' Populate Blood Lymphoma prevalence in synthetic population
#' 
...
#' @return input_population with added column: blood_lymphoma (year of diagnosis or 0)
#' @export
populate_blood_lymphoma_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Blood Lymphoma")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), blood_lymphoma)
  
  # Report summary
  n_cases <- sum(result$blood_lymphoma > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Blood Lymphoma", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Blood Lymphoma (10-year prevalence)
  pop_with_cancer <- populate_blood_lymphoma_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$blood_lymphoma > 0)
