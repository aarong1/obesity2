# =============================================================================
# Populate Stomach Cancer Prevalence
# =============================================================================
# Function to assign Stomach Cancer prevalence to a synthetic population
...
#' Populate Stomach Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: stomach_cancer (year of diagnosis or 0)
#' @export

populate_stomach_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Stomach Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), stomach_cancer)
  
  # Report summary
  n_cases <- sum(result$stomach_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Stomach Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Stomach Cancer (10-year prevalence)
  pop_with_cancer <- populate_stomach_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$stomach_cancer > 0)
