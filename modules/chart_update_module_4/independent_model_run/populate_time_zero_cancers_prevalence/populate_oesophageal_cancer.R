# =============================================================================
# Populate Oesophageal Cancer Prevalence
# =============================================================================
# Function to assign Oesophageal Cancer prevalence to a synthetic population
...
#' Populate Oesophageal Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: oesophageal_cancer (year of diagnosis or 0)
#' @export
populate_oesophageal_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Oesophageal Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), oesophageal_cancer)
  
  # Report summary
  n_cases <- sum(result$oesophageal_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Oesophageal Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Oesophageal Cancer (10-year prevalence)
  pop_with_cancer <- populate_oesophageal_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$oesophageal_cancer > 0)
  ...
  pop_with_cancer %>%
    ...
    summarise(
      n = n(),
      cases = sum(oesophageal_cancer > 0),
