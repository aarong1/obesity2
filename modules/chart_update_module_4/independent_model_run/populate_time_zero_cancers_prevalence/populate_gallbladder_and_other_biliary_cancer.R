# =============================================================================
# Populate Gallbladder And Other Biliary Cancer Prevalence
# =============================================================================
# Function to assign Gallbladder And Other Biliary Cancer prevalence to a synthetic population
...
#' Populate Gallbladder And Other Biliary Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: gallbladder_and_other_biliary_cancer (year of diagnosis or 0)
#' @export
populate_gallbladder_and_other_biliary_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Gallbladder And Other Biliary Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    mutate(
      gallbladder_and_other_biliary_cancer = if_else(
        has_cancer,
    ...
    select(all_of(names(input_population)), gallbladder_and_other_biliary_cancer)
  
  # Report summary
  n_cases <- sum(result$gallbladder_and_other_biliary_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Gallbladder And Other Biliary Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Gallbladder And Other Biliary Cancer (10-year prevalence)
  pop_with_cancer <- populate_gallbladder_and_other_biliary_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$gallbladder_and_other_biliary_cancer > 0)
  ...
  pop_with_cancer %>%
    ...
    summarise(
      n = n(),
      cases = sum(gallbladder_and_other_biliary_cancer > 0),
