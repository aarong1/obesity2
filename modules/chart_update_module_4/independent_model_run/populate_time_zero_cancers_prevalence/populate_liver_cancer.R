# =============================================================================
# Populate Liver Cancer Prevalence
# =============================================================================
# Function to assign Liver Cancer prevalence to a synthetic population
...
#' Populate Liver Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: liver_cancer (year of diagnosis or 0)
#' @export
populate_liver_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Liver Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), liver_cancer)
  
  # Report summary
  n_cases <- sum(result$liver_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Liver Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Liver Cancer (10-year prevalence)
  pop_with_cancer <- populate_liver_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$liver_cancer > 0)
