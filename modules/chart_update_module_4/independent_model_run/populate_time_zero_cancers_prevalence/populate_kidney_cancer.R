# =============================================================================
# Populate renal Cancer Prevalence
# =============================================================================
# Function to assign renal Cancer prevalence to a synthetic population
...
#' Populate renal Cancer prevalence in synthetic population
#' 
...
#' @return input_population with added column: renal_cancer (year of diagnosis or 0)
#' @export
populate_renal_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (is.null(cancer_prevalence_data)) {
    ...
    cancer_prevalence_data <- cancer_data %>%
      filter(cancer_site == "Kidney Cancer") %>%
      select(age, HSCT, sex, prevalence)
  }
  
  # Check that data exists
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for renal Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), renal_cancer)
  
  # Report summary
  n_cases <- sum(result$renal_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "renal Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with renal Cancer (10-year prevalence)
  pop_with_cancer <- populate_renal_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$renal_cancer > 0)
