# =============================================================================
# Populate Colorectal Cancer Prevalence
# =============================================================================
# Function to assign Colorectal Cancer prevalence to a synthetic population
...
#' Populate Colorectal Cancer prevalence in synthetic population

#' @param input_population Data frame with columns: id, age, sex, HSCT, year
...
#' @return input_population with added column: colorectal_cancer (year of diagnosis or 0)
#' @export

populate_colorectal_cancer_prevalence <- function(input_population, 
                                    period = "10-year",
  ...
  if (nrow(cancer_prevalence_data) == 0) {
    stop("No prevalence data found for Colorectal Cancer")
  }
  ...
  result <- pop_with_prob_calc %>%
    ...
    select(all_of(names(input_population)), colorectal_cancer)
  
  # Report summary
  n_cases <- sum(result$colorectal_cancer > 0)
  message(sprintf("%s: Assigned %d cases (%s prevalence)", 
                  "Colorectal Cancer", n_cases, period))
...
if (FALSE) {
  ...
  # Populate with Colorectal Cancer (10-year prevalence)
  pop_with_cancer <- populate_colorectal_cancer_prevalence(example_pop, period = "10-year")
  
  # Summary
  table(pop_with_cancer$colorectal_cancer > 0)
