...
qmortality_risk <- function( sex = NULL,
  ...
  if(sex == 'Males'){
    ...
    risk <- 2*qmortality_male_risk( age = age,
                          town = town, 
    ...
    risk <- 2*qmortality_female_risk(   age = age,
                              town = town, 
                              bmi = bmi,
                              alcohol_cat6 = alcohol_cat6, 
                              smoke_cat = smoke_cat, 
  ...
  # replace undefined ages with lifetables scores (absent deprivation)
...
apply_qmortality_mortality <- function(input_population, apply_death = F ){
  ...
  postp <- input_population |>
    ...
    rowwise() |> 
    mutate(qmortality_risk = qmortality_risk(age = age,
                                             sex = sex,
                                             town = townsend_score,
                                             bmi = bmi
                                             #b_CCF = heart_failure != 0,
                                             # b_asthmacopd = 1,
                                             #b_anycancer = lung_cancer != 0,
                                             #b_cvd = ifelse(chd != 0 | stroke != 0, 1, 0),
                                             #b_AF = atrial_fibrillation != 0,
