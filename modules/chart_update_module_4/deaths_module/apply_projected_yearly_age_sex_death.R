...
lifetables_male <- excel_sheets("data/lifetables.xlsx") %>%
  .[5:17] |> 
  set_names() %>%                                   # keep sheet name as list name
  map_df(~ read_excel(
    "data/lifetables.xlsx",
    sheet = .x,
    range = "A6:F107"
  ) %>%
    mutate(sheet = .x),                             # add sheet name column
  .id = "period"                                  # optional: store sheet name as 'period'
  )
...
lifetables_female <- excel_sheets("data/lifetables.xlsx") %>%
  .[5:17] |> 
  set_names() %>%                                   # keep sheet name as list name
  map_df(~ read_excel(
    "data/lifetables.xlsx",
