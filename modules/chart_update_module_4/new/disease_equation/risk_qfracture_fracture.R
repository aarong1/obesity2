...
# QFracture-2016 R implementation
...
default_fracture4_female <- function(
    age,
    bmi = NULL,
    ethrisk = NULL,
    smoke_cat = 1,
  ...
  # Coefficient arrays
  Ialcohol <- c(0, -0.016119659842711558, 0.018142191954688299,
                0.087039813091311105, 0.48508766816483712, 0.4521470045723863)
  Iethrisk <- c(0, 0, -0.42566069216366254, -0.55432091195021416,
                -0.91826010978069306, -0.68193606531483042, -1.4668483404988077,
                -0.9101238114228446, -0.6421783317544739, -0.5036829432634511)
  Ismoke   <- c(0, 0.05573569343056117, 0.16338956617013528,
  ...
  # Analogous: Fracture risk (4-year) for males
# returns probability (0–1) with optional interventions

default_fracture4_male <- function(
    age = NULL,
    bmi = NULL,
    ethrisk = NULL,
    smoke_cat = 1,
...
# Unified 4-year fracture risk function
risk_qfracture_fracture4 <- function(
    ...
    b_anycancer = 0,
    b_asthmacopd = 0,
    b_carehome = 0,            # male-only; ignored for female
    b_corticosteroids = 0,
    b_cvd = 0,
    b_dementia = 0,
    b_endocrine = 0,           # female-only; ignored for male
    b_epilepsy2 = 0,
    b_falls = 0,
    b_hrt_oest = 0,            # female-only; ignored for male
    b_liver = 0,
    b_malabsorption = 0,
    b_parkinsons = 0,
    b_ra_sle = 0,
  ...
  if (is.null(age))    stop("'age' is mandatory.", call. = FALSE)
  if (is.null(bmi))    stop("'bmi' is mandatory.", call. = FALSE)
  if (is.null(ethrisk))stop("'ethrisk' is mandatory.", call. = FALSE)
  ...
  if (sex == "Females") {
    # female-only args
    args$b_anycancer  <- b_anycancer
    args$b_endocrine   <- b_endocrine
    args$b_hrt_oest    <- b_hrt_oest
    # drop male-only
    args$b_carehome    <- NULL
    fn <- default_fracture4_female
  } else {
    # male-only args
    args$b_anycancer  <- b_anycancer
    args$b_carehome   <- b_carehome
    # drop female-only
    args$b_endocrine  <- NULL
    args$b_hrt_oest   <- NULL
    fn <- default_fracture4_male
  }
...
risk_qfracture_fracture4(   sex="Females",
                            age=65, 
                            bmi=22, 
                            ethrisk = 3 )
...
# Wrapper to apply 4-year overall fracture risk
apply_fracture4_risk_wo_risk_factors <- function(
  ...
  input_population <- input_population %>%
    select( ! any_of('fracture_risk') ) %>%
    left_join(
      select(postp, id, fracture_risk),
      by = "id"
    )
...
# 
#   test_population |> 
#   apply_fracture4_risk_wo_risk_factors() |> names()
#   pull(fracture_risk)
  
  # ggplot()+
  # geom_point(aes(age,fracture_risk,col = as.character(broad_ethnicity) %in% 'white'))
