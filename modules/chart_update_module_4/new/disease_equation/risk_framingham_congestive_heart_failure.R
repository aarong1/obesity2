  ...
  postp <- postp1 %>%
    mutate(
      ...
      bmi = case_when(
        ...
        bmi == "obese"       ~ 40,
        TRUE ~ NA_real_),
      LVH_b = (heart_failure != 0),
      Systolic_blood_pressure_20_mm_Hg = case_when(hypertension_status %in%
                                                     c( 'hypertensive_uncontrolled',
                                                        'hypertensive_untreated') ~ 150/20,
                                                   T~ 110/20),
      CHD_b =  (chd != 0 ),
      Diabetes_b = (diabetes_status != 'no_diabetes')
      ) 
    ...
    mutate(hf_risk =
             ifelse(age < 35, 0,
                    heart_failure_risk(
                      ...
                      BMI_kg_m2 = bmi,
                      LVH_b = LVH_b,
                      Systolic_blood_pressure_20_mm_Hg = Systolic_blood_pressure_20_mm_Hg,
                      CHD_b = CHD_b,
                      Diabetes_b = Diabetes_b
                    )
             )
    ) |> 
###########################################################
