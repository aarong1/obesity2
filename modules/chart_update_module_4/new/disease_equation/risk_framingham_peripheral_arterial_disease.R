...
apply_pad_risk_wo_risk_factors <- function(input_population){
  ...
  postp <- postp1 %>%
    mutate(
      ...
      sex,
      bp_cat  = case_when(
        hypertension_status == "normotensive_untreated" ~ 'Normal' ,
        hypertension_status == "hypertensive_controlled" ~ 'High Normal' ,
        hypertension_status == "hypertensive_uncontrolled" ~ 'Stage1 Hypertension' ,
        hypertension_status == "hypertensive_untreated" ~ 'Stage1 Hypertension'
      ), 
      diabetes = (diabetes != 'no_diabetes'),
      ...
      cholesterol =
        case_when(
          cholesterol_status == 'normal_cholesterol' ~ 190,
          cholesterol_status == 'hdl/cholesterol' ~ 240,
          cholesterol_status == 'raised_cholesterol' ~ 280
        ),
      chd = (chd!=0)
      )
