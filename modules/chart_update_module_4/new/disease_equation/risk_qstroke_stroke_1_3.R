  ...
  postp <- postp1 %>%
      mutate(
        ...
        bmi = case_when(
          ...
          bmi == "obese"       ~ 38,
          TRUE ~ NA_real_),
        cholesterol_ratio = case_when(
          cholesterol_status == "normal_cholesterol"  ~ 4,
          cholesterol_status == "raised_cholesterol"  ~ 5,
          cholesterol_status == "hdl/cholesterol"     ~ 6,
          TRUE ~ NA_real_),
        b_type2 = case_when(
        ...
        sbp =case_when(
          hypertension_status == "hypertensive_untreated" ~ 150,
          hypertension_status == "hypertensive_uncontrolled" ~ 140,
          hypertension_status == "hypertensive_controlled" ~ 120,
          TRUE ~ 120),
        b_treatedhyp = case_when(hypertension_status == 'hypertensive_uncontrolled'~1,
                                 hypertension_status == 'hypertensive_controlled'~1,
                                 T~0),
    ...
    mutate(stroke_risk =
             ifelse(age < 30, 0,
                    risk_qstroke_stroke(
                         ...
                         b_CCF = b_CCF,
                         b_AF = af_status!= 'no_af',
                         b_chd = chd!=0,
                         b_ra = rheumatoid_arthritis!=0,
