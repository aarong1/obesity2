...
apply_stroke_risk_wo_risk_factors1 <- function(input_population,intervention=1){
  ...
  postp <- postp1 %>%
      mutate(
        ...
        cholesterol_ratio = case_when(
          cholesterol_status == 'raised_cholesterol' ~ 6,
          cholesterol_status == 'normal_cholesterol' ~ 4.5,
          TRUE ~ 4.5
        ),
        sbp = case_when(hypertension_status %in% c( 'hypertensive_uncontrolled',
                                                   'hypertensive_untreated') ~ 150,
                       T~ 110),
        atrial_fibrillation = (atrial_fibrillation !=0),
        ethrisk = ethrisk,
        b_chd = (chd !=0),
        b_ra = (rheumatoid_arthritis!=0),
        b_renal = (chronic_kidney_disease!=0),
        b_treatedhyp = (hypertension_status %in% c('hypertensive_controlled',
                                                  'hypertensive_treated')),
