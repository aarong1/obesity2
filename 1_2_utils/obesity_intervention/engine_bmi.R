...
smoking_down_one_level <- function(input_population,
                               subset = FALSE,
                               subset_name = 'target') {
  if(subset==TRUE){
    ...
    input_population <- input_population %>%
      mutate(
        smoking = case_when(
          smoking=='current_smoker' & target == TRUE ~ "former",
          smoking=='former'  & target == TRUE~ "never_smoked",
          smoking=='never_smoked'  & target == TRUE~ "never_smoked",
          
          TRUE ~ smoking  # Keep original if something unexpected
        )
      )
    ...
    input_population <- input_population %>%
      mutate(
        smoking = case_when(
          smoking=='current_smoker'  ~ "former",
          smoking=='former'  ~ "never_smoked",
          TRUE ~ smoking  # Keep original if something unexpected
        )
      )
  }
...
bmi_down_one_level <- function(input_population,
                               subset = FALSE,
                               subset_name = 'target') {
  if(subset==TRUE){
...
bmi_obese_overweight <- function(input_population,
                               subset = FALSE,
                               subset_name = 'target') {
  if(subset==TRUE){
...
reduce_all_percentile <- function(input_population,
                                  subset = FALSE,
                                  by=0.8,
                                  subset_name = 'target' ){
  ...
  percentile_cols <- grep("_percentile$", names(input_population), value = TRUE)
  
  if(subset==TRUE){
  input_population[get(subset_name)==T, (percentile_cols) := lapply(.SD, function(x) {
    x * by
  }), .SDcols = percentile_cols]
  } else{
    input_population[, (percentile_cols) := lapply(.SD, function(x) {
      x * by
    }), .SDcols = percentile_cols]
  }
}

reduce_bmi_percentile <- function(input_population,
                                  by=0.8,
                                  subset_name = 'target' ){
  ...
  percentile_cols <- grep("_percentile$", names(input_population), value = TRUE)
    ...
    input_population[, bmi_percentile := bmi_percentile * by,
                     .SDcols = percentile_cols]
  
}


# count(past_populations, wt=bmi_percentile)*0.8

# count(reduce_all_precentile(past_populations), wt=bmi_percentile)*0.8

# adherence_fraction = 0.9
# 
# current_population %>% 
#   # comorbidity_status
#   mutate(target = ifelse(diabetes_status == 'diagnosed_diabetes',T,F )) %>%
#   # deprivation status
#   mutate(target = ifelse(mdm_quintile_soa_name == 'Most Deprived',T,F )) %>%
#   # geography
#   mutate(target = ifelse(HSCT == 'NHSCT',T,F )) %>%
#   # comorbidity_status and deprivation
#   mutate(target = ifelse(mdm_quintile_soa_name == 'Most Deprived' & 
#                            diabetes_status == 'diagnosed_diabetes',T,F )) %>%
...
#   count(bmi)
# 
# counting_intervention <- current_population %>% 
#   mutate(target = ifelse(mdm_quintile_soa_name == 'Most Deprived' & 
#                            diabetes_status == 'diagnosed_diabetes',T,F ))
# 
# sum(counting_intervention$target)
# nrow(counting_intervention)
# paste((sum(counting_intervention$target)/ nrow(counting_intervention)) %>% round(3) * 100, '%')
# 
# 
# adhere_counting_intervention <- counting_intervention %>% 
#   mutate(target = ifelse(target ==T, target *runif(n())/(adherence_fraction/0.5)<0.5,F)) %>%
#   pull(target) %>% 
#   sum()
#   
# abs(adhere_counting_intervention-sum(counting_intervention$target))
# 
# abs(adhere_counting_intervention-sum(counting_intervention$target))/adhere_counting_intervention
