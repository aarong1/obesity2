...
death_rates_by_age <- total_deaths %>%
  ...
  mutate(
    ...
    qx_suicide =  suicide_deaths / population, 0,
    qx_not_suicide =  (total_deaths -suicide_deaths) / population, 0,
    
    qx_natural = natural_deaths / population, 0,
    qx_not_natural = (total_deaths -natural_deaths) / population, 0,
    
    qx_total =  total_deaths / population, 0,
    qx_surv =  (population-total_deaths) / population, 0
  )
...
# 10 Females 8       4.55   4.29   4.14   4.01   3.89   3.79   3.71   3.63   3.56   3.49
 ...
 y <-  pop %>% 
  ...
  mutate(q_modelled_other = q_modelled_all_cause - q_modelled_natural - q_modelled_suicide) %>% filter(q_modelled_other<0) 
...
pop_d = data.frame(); pop_a = data.frame(); for( k in  1:2){ #1:20
  ...
  for(j in c('intervention','non-intervention')){
    ...
    for( i in 2023:2040){#2023:2050
      ...
      pop2 <- pop2 %>% 
        rowwise() %>% 
        mutate(q_all = list(list(q_modelled_other = q_modelled_other/2,
                                 q_modelled_natural = q_modelled_natural/2,
                                 q_modelled_suicide = q_modelled_suicide/2,
                                 q_surv = q_surv))) %>%
