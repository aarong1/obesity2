...
apply_child_bmi_lifestyle_parameter_3State_rank_stability <-function(current_population,
  ...
  missing_kid_bmi <- sum(is.na(current_population$child_bmi))
  if(missing_kid_bmi > 0) {
    warning(paste("Warning:", missing_kid_bmi, "individuals could not be assigned a child BMI category. Check for missing combinations in bmi_stratified_prevalence."))
  }
  
  current_population <- current_population |> 
    select(-c( prob_cum,probability))
}
