...
apply_granular_cholesterol_measure_posthoc_overlay <- function(current_population,special_cholesterol){
  ...
  if(! 'cholesterol_status' %in% names(current_population)){
    stop('cholesterol_status not found')
  }
  
  
  missing_cholesterol_status <- sum(is.na(current_population$cholesterol_status))
  if(missing_cholesterol_status > 0) {
    warning(paste("Warning:", missing_cholesterol_status, "individuals could not be assigned an cholesterol_status category. Check for missing combinations in alcohol_stratified_prevalence."))
  }
  ...
  current_population <- current_population %>% 
    ...
    select(-any_of(c('prob_cond', 'prob_cum', 'list_probs','d', 'probability', 'cholesterol_status.y')))
