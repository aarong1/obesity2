...
apply_wellbeing_depression_lifestyle_parameter_rank_stability <- function(current_population,
  ...
  current_population <- current_population %>% 
    select(-any_of('wellbeing'))  # Remove existing wellbeing column if present
  ...
  current_population <- current_population |> 
    mutate(deprivation = as.character(mdm_quintile_soa_name) #case_when(
  ...
  current_population <- current_population |> 
   select( - any_of(c( 'list_probs', 'percentage','percentage.x', 'percentage.y', 'probability', 'prob_cum', 'prob_cond' ) ))
