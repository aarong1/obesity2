...
apply_diabetes_physiological_parameter_rank_stability <- function(current_population,
  ...
  diabetes_stratified_prevalence_df <- diabetes_joint_estimation_results_df |> 
    rename(diabetes_status = diabetes) %>% 
  ...
  current_population <- current_population |> 
    select( -  any_of(c( 'list_probs', 'probability', 'prob_cum', 'prob_cond' ) )) #percentage
  
  
  
  missing_diabetes_status <- sum(is.na(current_population$diabetes_status))
  if(missing_diabetes_status> 0) {
    warning(paste("Warning:", missing_diabetes_status, "individuals could not be assigned an diabetes_status category. Check for missing combinations in alcohol_stratified_prevalence."))
  }
